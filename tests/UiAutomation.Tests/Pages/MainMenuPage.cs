...
public sealed class MainMenuPage(IWebDriver driver, TimeSpan waitTimeout)
{
    ...
    /// <summary>
    /// Пункт меню ищется по тексту ссылки внутри панели: у пунктов нет ни
    /// идентификаторов, ни собственных классов, а href у половины содержит UUID
    /// аккаунта, который меняется от загрузки к загрузке.
    /// </summary>
    /// <remarks>
    /// Кроме точного совпадения принимается имя, за которым идёт продолжение
    /// через пробел: у части пунктов приложение дописывает счётчик — «Повернення»
    /// превращается в «Повернення - 1», когда есть незакрытые возвраты. Счётчик
    /// появляется и пропадает вместе с данными, поэтому по точному тексту такой
    /// пункт то находится, то нет.
    ///
    /// Ни одно имя из эталонного состава не является началом другого, так что
    /// послабление не даёт пункту совпасть с соседом.
    /// </remarks>
    private static By MenuItemBy(string itemName) =>
        By.XPath(
            $"{SideMenuPath}//a[normalize-space(.)={XPathHelpers.Literal(itemName)} " +
            $"or starts-with(normalize-space(.), {XPathHelpers.Literal(itemName + " ")})]");
}
