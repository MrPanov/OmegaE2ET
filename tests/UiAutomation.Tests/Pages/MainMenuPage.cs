...
public sealed class MainMenuPage(IWebDriver driver, TimeSpan waitTimeout)
{
    ...
    /// </summary>
    /// <remarks>
    /// Совпадение по вхождению, а не по всему тексту: к части пунктов приложение
    /// дописывает счётчик — «Повернення» превращается в «Повернення - 1», когда
    /// есть незакрытые возвраты. Счётчик появляется и пропадает вместе с данными,
    /// поэтому по точному тексту такой пункт то находится, то нет.
    ///
    /// Соседей это не задевает: сравнение чувствительно к регистру, и «Кошик
    /// повернень» под «Повернення» не подходит.
    /// </remarks>
    private static By MenuItemBy(string itemName) =>
        By.XPath(
            $"{SideMenuPath}//a[contains(normalize-space(.), {XPathHelpers.Literal(itemName)})]");
}
