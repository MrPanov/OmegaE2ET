...
Basket-сценарии находятся в `BasketAddProductTests`. Тест добавляет товар по номеру
карточки через поле `Додати позицію` и проверяет, что позиция появилась ровно одной
строкой; набор карточек задан параметрами `TestCase`. Фикстура помечена
`ProductionTestClient` и `MutatesUserState`, получает новую браузерную сессию для
каждого теста и убирает за собой: товар удаляется в `TearDown`, а перед добавлением
снимается остаток от предыдущего прогона, если он был. Сбой уборки пишется в вывод
теста и не роняет прошедшую проверку.

Остальные ручные сценарии корзины — добавление по каталожному коду, количество,
удаление позиции и очистка корзины — описаны в `docs/manual-tests/basket.md`
и пока не автоматизированы.
