...
`tests/UiAutomation.Tests/Tests/SearchTests.cs`. Они покрывают поиск по коду и
карточке, регистронезависимость, пустую выдачу и замену предыдущего результата.
Цены и остатки намеренно не фиксируются, поскольку являются динамическими.
